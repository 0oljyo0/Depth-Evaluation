...
with tf.name_scope("conv1"):
    ...
    conv_temp = tf.nn.relu(conv2d(x, filter) + bias)
    conv_temp = tf.layers.batch_normalization(conv_temp,training=True)
    conv_res1 = max_pool_2_2(conv_temp)
    print(conv_res1.shape)
    
with tf.name_scope("conv2"):
    filter = weight_variable([3,3,32,64])
    bias = bias_variable([64])
    conv_temp = tf.nn.relu(conv2d(conv_res1, filter) + bias)
    conv_temp = tf.layers.batch_normalization(conv_temp,training=True)
    conv_res2 = max_pool_2_2(conv_temp)
    print(conv_res2.shape) 
    
with tf.name_scope("conv3"):
    filter = weight_variable([3,3,64,128])
    bias = bias_variable([128])
    conv_temp = tf.nn.relu(conv2d(conv_res2, filter) + bias)
    conv_temp = tf.layers.batch_normalization(conv_temp,training=True)
    conv_res3 = max_pool_2_2(conv_temp)
    print(conv_res3.shape)
    
with tf.name_scope("conv4"):
    filter = weight_variable([3,3,128,256])
    bias = bias_variable([256])
    conv_temp = tf.nn.relu(conv2d(conv_res3, filter) + bias)
    conv_temp = tf.layers.batch_normalization(conv_temp,training=True)
    conv_res4 = max_pool_2_2(conv_temp)
    print(conv_res4.shape) 
    
with tf.name_scope("conv5"):
    filter = weight_variable([3,3,256,512])
    bias = bias_variable([512])
    conv_temp = tf.nn.relu(conv2d(conv_res4, filter) + bias)
    conv_temp = tf.layers.batch_normalization(conv_temp,training=True)
    conv_res5 = max_pool_2_2(conv_temp)
...
with tf.name_scope("deconv5"):
    ...
    # deconv_res1 = tf.layers.batch_normalization(conv_temp,training=True)
